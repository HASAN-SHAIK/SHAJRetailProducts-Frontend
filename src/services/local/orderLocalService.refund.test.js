...
describe('local POS full-refund contract', () => {
  ...
  test('routes a full-sale refund and signals refreshed diagnostics', async () => {
    ...
    expect(signalRefundDiagnosticsRefresh).toHaveBeenCalledWith('ord-1', 'full_refund_succeeded');
  ...
  test('signals reconciliation facts immediately when refund is blocked', async () => {
    ...
    await expect(refundOrder('ord-reconcile', { reason: 'Full return' }))
