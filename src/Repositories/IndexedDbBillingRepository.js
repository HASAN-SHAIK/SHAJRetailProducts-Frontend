...
/** @implements {import('../Interfaces/IBillingRepository').IBillingRepository} */
export class IndexedDbBillingRepository {
  ...
  async getOrdersByStatus(status, transactionType = 'sale') {
    if (!status) return [];
  ...
  async updateSyncQueueEntry(entry) {
  ...
  async findPendingSyncEntry(orderId, action) {
    if (!orderId || !action) return null;
    const pending = await this.getPendingSyncEntries();
