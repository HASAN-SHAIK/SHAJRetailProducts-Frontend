...
const resolveOrderType = (order = {}) => normalizeTransactionType(order);
...
export class IndexedDbOrderRepository {
  ...
  async getCachedOrdersByType(type = 'sale') {
  ...
  async replaceCachedOrdersByType(type = 'sale', orders = []) {
    const table = getOrdersTableByType(type);
    const list = Array.isArray(orders) ? orders : [];
    const normalized = list.map(normalizeOrder).filter((item) => item.id !== null);
  ...
  async clearCachedOrdersByType(type = 'sale') {
    const table = getOrdersTableByType(type);
    await table.clear();
  }

  async getCachedOrderTransactions(orderId) {
    const key = normalizeOrderKey(orderId);
    if (key === null) return [];
  ...
  async upsertOrderDetailsCache({ order, items, payments } = {}) {
    if (!order) return;
    await db.transaction('rw', db.orders, db.sales_orders, db.purchase_orders, db.order_items, db.transactions, async () => {
      await this.upsertOrders([order]);
      if (order?.id && Array.isArray(items) && items.length) {
        await this.replaceCachedOrderItems(order.id, items);
      }
      if (order?.id) {
  ...
  async getCachedOrderDetails(orderId) {
    const order = await this.getCachedOrderById(orderId);
    if (!order) return null;
    const items = await this.getCachedOrderItems(orderId);
    const payments = await this.getCachedOrderTransactions(orderId);
    return {
      ...order,
  ...
  async getCachedOrdersPage({ page = 1, limit = 20 } = {}) {
  ...
  async getCachedOrdersByCustomer(customer = {}) {
    ...
    return db.order_items
      ...
      .then((byText) => {
        ...
        if (Number.isFinite(idNum)) {
          return db.order_items.where('order_id').equals(idNum).toArray();
        }
        return byText;
      });
  }
  ...
  getSalesOrderRecords() {
    return db.sales_orders.toArray();
  }

  getPurchaseOrderRecords() {
    return db.purchase_orders.toArray();
  }

  bulkPutSalesOrders(rows) {
    return db.sales_orders.bulkPut(rows);
  }

  bulkPutPurchaseOrders(rows) {
    return db.purchase_orders.bulkPut(rows);
  }

  clearSalesOrders() {
    return db.sales_orders.clear();
  }

  clearPurchaseOrders() {
    return db.purchase_orders.clear();
  }

  getSalesAndPurchaseOrderCounts() {
    return Promise.all([db.sales_orders.toArray(), db.purchase_orders.toArray()]);
  }

  async listOrders() {
    throw new Error('Order list requires an online SQL connection');
  }

  async getOrderDetail() {
    throw new Error('Order detail requires an online SQL connection');
  }

  async createOrder() {
    throw new Error('Order create requires an online SQL connection');
  }

  async updateOrder() {
    throw new Error('Order update requires an online SQL connection');
  }

  async deleteOrder() {
    throw new Error('Order delete requires an online SQL connection');
  }

  async syncOfflineOrders() {
    throw new Error('Offline order sync requires an online SQL connection');
  }

  async markOrderPaid() {
    throw new Error('Mark paid requires an online SQL connection');
  }

  async createOrderReturn() {
    throw new Error('Order return requires an online SQL connection');
  }
  ...
  async createCorrection() {
    throw new Error('Correction create requires an online SQL connection');
  }

  async deleteReturn() {
    throw new Error('Return delete requires an online SQL connection');
  }

  async upsertReturn() {
    throw new Error('Return upsert requires an online SQL connection');
  }

  async deleteCorrection() {
    throw new Error('Correction delete requires an online SQL connection');
  }

  async upsertCorrection() {
    throw new Error('Correction upsert requires an online SQL connection');
  }
}
